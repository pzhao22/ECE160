...
Player::Player(int n, int m = 1500, int pn = 0, vector<Square*> p = {})
{
  number = n;
  money = m;
  position = pn;
...
void make_deal(int p)
{
  ...
  if (offer == 'M' || offer == 'm')
  {
    ...
    if(playerVector[p].money > amount)
    {
      string property;
  ...
  else if (offer == 'P' || offer == 'p')
  {
    ...
    else
    {
      ...
      if(playerVector[index].money > amount)
      {
        char agreement;
        cout << "Both parties agree? (y/n): ";
        cin >> agreement;
  ...
  char again;
  cout << "Would you like to make another deal? (y/n): ";
  cin >> again;
  if (again == 'Y' || again == 'y')
  {
    make_deal(p);
